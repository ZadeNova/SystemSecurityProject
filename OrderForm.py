from wtforms import Form, StringField, RadioField, TimeField, SelectField, TextAreaField, validators, IntegerField, \
...
class OrderInventory(Form):
    Quantity = IntegerField('Quantity', [validators.NumberRange(min=1), validators.DataRequired()])
    Date = DateTimeField("Date and Time", format='%d/%m/%y',default=date.today())
