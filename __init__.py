    ...
    if request.method == 'POST' and 'Username' in request.form and 'Password' in request.form and 'Email' in request.form and create_login_user_form.validate():
        if 'terms-and-conditions' in request.form:
            if is_human(captcha_response):
                ...
                return render_template('create_login_user_form.html', msg=msg, form=create_login_user_form, sitekey="6LeQDi8bAAAAAGzw5v4-zRTcdNBbDuFsgeU2jEhb")
        else:
...
def OrdersCreate(id):
    ...
    try:
        ...
        if inventory.get_Category() == 'Meat':
            ...
            OrderCreationForm.Supplier.choices = [NecessitiesSupplierChoice[a] for a in range(NecessitiesSupplierLength)]
...
def ViewOrders():
    ...
    return render_template('ViewOrders.html', OrderCount=len(Order_List), Order_List=Order_List, name=username, role=role)
